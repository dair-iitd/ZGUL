...
class ModelAdaptersMixin(PushAdapterToHubMixin, ABC):
    ...
    def train_adapter_fusion_TA(self, adapter_setup_TA:Union[list, AdapterCompositionBlock], adapter_setup_LA: Union[list, AdapterCompositionBlock], train_embeddings=False, unfreeze_adapters=False):
        """TA"""
        self.train()
        #pdb.set_trace()
        self.freeze_model(True)
        #import pdb
        #pdb.set_trace()
        adapter_setup_TA = parse_composition(adapter_setup_TA)
        #import pdb
        self.apply_to_adapter_layers(lambda i, layer: layer.enable_adapters(adapter_setup_TA, True, False))
        for adapter_name in adapter_setup_TA:
            if adapter_name in self.shared_parameters:
                for param in self.shared_parameters[adapter_name].values():
                    param.requires_grad = True

        if isinstance(self, InvertibleAdaptersMixin):
            self.enable_invertible_adapters(adapter_setup_TA.flatten())
        # use the adapters to be trained by default in every forward pass
        self.set_active_adapters(adapter_setup_TA)
        ...
        """LAs fused"""
        adapter_setup_LA = parse_composition(adapter_setup_LA)
        self.apply_to_adapter_layers(lambda i, layer: layer.enable_adapters(adapter_setup_LA, unfreeze_adapters, True))
        # use the adapters to be trained by default in every forward pass
        
        self.set_active_adapters(adapter_setup_LA)
        if hasattr(self, "roberta"):
            for param_ in self.roberta.l2v_mlp3.parameters():
                param_.requires_grad = True
        else:
            for param_ in self.bert.l2v_mlp3.parameters():
                param_.requires_grad = True
            #for param_ in self.bert.pad_vecs:
            #    param_.requires_grad = True
        #self.bert.pad_vecs.requires_grad=True
    # def train_adapter_fft(self, adapter_setup_TA:Union[list, AdapterCompositionBlock], adapter_setup_LA: Union[list, AdapterCompositionBlock], train_embeddings=True, unfreeze_adapters=False):
    #     """TA"""
    #     self.train()
    #     #pdb.set_trace()
    #     #self.freeze_model(True)
    ...
    def train_fusion_freeze_TA(self, adapter_setup_TA:Union[list, AdapterCompositionBlock], adapter_setup_LA: Union[list, AdapterCompositionBlock], train_embeddings=False, unfreeze_adapters=False):
        """TA"""
        self.train()
        #pdb.set_trace()
        self.freeze_model(True)
        ...
        adapter_setup_TA = parse_composition(adapter_setup_TA)
        #import pdb
        self.apply_to_adapter_layers(lambda i, layer: layer.enable_adapters(adapter_setup_TA, False, False))
        for adapter_name in adapter_setup_TA:
            if adapter_name in self.shared_parameters:
                for param in self.shared_parameters[adapter_name].values():
                    param.requires_grad = True
    ...
    def train_adapter_fft_no_TA(self, adapter_setup_TA:Union[list, AdapterCompositionBlock], adapter_setup_LA: Union[list, AdapterCompositionBlock], keys_orig_model, train_embeddings=True, unfreeze_adapters=False):
        """TA"""
        self.train()
        self.freeze_model(True)
        """LAs fused"""
