...
class Adapter(nn.Module):
    ...
    def forward(self, x, residual_input, layer_id):  # , residual_input=None):
        #pdb.set_trace()
        #if layer_id == 11:
        #    pdb.set_trace()
...
class BertFusion(nn.Module):
    ...
    def __init__(
        ...
        #self.linear_l2v1 = nn.Linear(160, 512)
        self.l2v_trans = nn.Linear(32,32)
        #self.q_lang_pad = torch.zeros(1,1,32)
        self.linear_final = nn.Linear(2*768, 768)

    def forward(self, query, key, value, residual, lang_vecs, layer_id=-1, adapter_weights=None, attention_mask=None):
        #pdb.set_trace()
        #if layer_id == 11:
        #    pdb.set_trace()
        ...
        batch_, max_len_ = query_layer.shape[0], query_layer.shape[1]

        if adapter_weights is None:
            attention_scores_1 = torch.squeeze(torch.matmul(query_layer.unsqueeze(2), key_layer.transpose(-2, -1)), dim=2)
            #pdb.set_trace()
            q_lang=self.l2v_trans(lang_vecs[:,0,:]).unsqueeze(1)
            q_lang_pad=self.l2v_trans(lang_vecs[:,-1,:]).unsqueeze(1)
            ...
            attention_scores_1 = self.dropout(attention_scores_1)
            attention_probs_1 = nn.Softmax(dim=-1)(attention_scores_1/self.T)
            self.T = max(self.T - self.reduction, 1.0)
            #if layer_id == 0:
            #    print(attention_probs_2[0][0], attention_probs_2[0][-1])
        else:
            #pdb.set_trace()
            attention_probs_1, attention_probs_2 = torch.split(adapter_weights, batch_, 0)
            #attention_probs_1 = adapter_weights.unsqueeze(1).repeat(1,128,1).type(torch.FloatTensor).to("cuda:0")
            #attention_probs_2 = adapter_weights.unsqueeze(1).repeat(1,128,1).type(torch.FloatTensor).to("cuda:0")
            #print(attention_probs_1[0][0], attention_probs_2[0][0])
            #attention_probs_2 = adapter_weights[1]

        if not self.training:
            self.recent_attention = attention_probs_2.detach().cpu().numpy()

        context_layer_fusion = torch.squeeze(torch.matmul(attention_probs_1.unsqueeze(2), value_layer), dim=2)
        context_layer_l2v = torch.squeeze(torch.matmul(attention_probs_2.unsqueeze(2), value_layer), dim=2)
        #context_layer_cpg = self.cpg_adapter(query_layer, residual, lang_vecs)
        #pdb.set_trace()
        context_layer = self.linear_final(torch.cat((context_layer_fusion, context_layer_l2v),dim=-1))
...
class BertFusionGermanic(nn.Module):
    """
    Implementation of an AdapterFusion block.
    """
    ...
    def __init__(
        ...
        super(BertFusionGermanic, self).__init__()
        # if config.hidden_size % config.num_attention_heads != 0:
        #     raise ValueError(
        #         "The hidden size (%d) is not a multiple of the number of attention "
        #         "heads (%d)" % (config.hidden_size, config.num_attention_heads))
        ...
        if self.config["temperature"]:
            ...
            self.T = 1.0
        self.reduction = self.T / 1000.0
        self.linear_l2v1 = nn.Linear(128, 384)
        #self.l2v_trans = nn.Linear(32,32)
        #self.q_lang_pad = torch.zeros(1,1,32)
        self.lang_dim = 32 
        self.query_proj = nn.Linear(self.lang_dim, self.lang_dim, bias=False)
        self.key_proj = nn.Linear(self.lang_dim, self.lang_dim, bias=False)
        self.bias = nn.Parameter(torch.rand(self.lang_dim).uniform_(-0.1, 0.1))
        self.score_proj = nn.Linear(self.lang_dim, 1)
    ...
    def forward(self, query, key, value, residual, lang_vecs, layer_id=-1, adapter_weights=None, attention_mask=None):
        ...
        if adapter_weights is None:
            ...
            k_lang=lang_vecs[:,1:-1,:]
            #pdb.set_trace()
            lang_score1 = nn.Softmax(dim=-1)(self.score_proj(torch.tanh(self.key_proj(k_lang) + self.query_proj(q_lang) + self.bias)).squeeze(-1).unsqueeze(1).repeat(1,max_len_,1))
            #lang_score1 = nn.Softmax(dim=-1)(torch.randn(4).to('cuda:0').unsqueeze(0).unsqueeze(0).repeat(batch_, max_len_,1))
            #lang_score2 = nn.Softmax(dim=-1)(torch.randn(4).to('cuda:0').unsqueeze(0).unsqueeze(0).repeat(batch_, max_len_,1))
            lang_score2 = nn.Softmax(dim=-1)(self.score_proj(torch.tanh(self.key_proj(k_lang) + self.query_proj(q_lang_pad) + self.bias)).squeeze(-1).unsqueeze(1).repeat(1,max_len_,1))
            mask = (attention_mask==0.0).squeeze().float().unsqueeze(-1).repeat(1,1,3)
            attention_probs_2=mask*lang_score1+(1-mask)*lang_score2
            #print(attention_probs_2[0][-1])
            #print(lang_vecs[:,0,:][0]) 
            #cat_scores = torch.cat((attention_scores_1.unsqueeze(-1), attention_scores_2.unsqueeze(-1)),-1)
            attention_scores_1 = self.dropout(attention_scores_1)
            #attention_scores_2 = self.dropout(attention_scores_2)
            attention_probs_1 = nn.Softmax(dim=-1)(attention_scores_1/self.T)
            #attention_probs_2 = self.dropout(attention_scores_2)
            self.T = max(self.T - self.reduction, 1.0)
        ...
        if not self.training:
            self.recent_attention = attention_probs_1.detach().cpu().numpy()
...
class BertFusionSlavic(nn.Module):
    """
    Implementation of an AdapterFusion block.
    """
    ...
    def __init__(
        ...
        super(BertFusionSlavic, self).__init__()
        # if config.hidden_size % config.num_attention_heads != 0:
        #     raise ValueError(
        #         "The hidden size (%d) is not a multiple of the number of attention "
        #         "heads (%d)" % (config.hidden_size, config.num_attention_heads))
    ...
    def forward(self, query, key, value, residual, lang_vecs, layer_id=-1, adapter_weights=None, attention_mask=None):
        ...
        #context_layer_cpg = self.cpg_adapter(query_layer, residual, lang_vecs)
        #pdb.set_trace()
        context_layer = torch.max(torch.cat((context_layer_fusion.unsqueeze(-1),context_layer_l2v.unsqueeze(-1)), dim=-1), -1)[0]
