...
class AdapterLayer(AdapterLayerBase):
    ...
    def add_fusion_layer(self, adapter_names: Union[List, str]):
        ...
        if self.config.adapters.common_config_value(adapter_names, self.location_key):
            ...
            #pdb.set_trace()
            if 'de' in adapter_names:
                fusion = BertFusionGermanic(
                    fusion_config,
                    self.config.hidden_size,
                    self.config.attention_probs_dropout_prob,
                )
            elif 'ru' in adapter_names:
                fusion = BertFusionSlavic(
                    fusion_config,
                    self.config.hidden_size,
                    self.config.attention_probs_dropout_prob,
                )
    ...
    def adapter_stack(self, adapter_setup: Stack, hidden_states, input_tensor, layer_norm, lang_vecs, layer_id, adapter_names=None, adapter_weights=None, attention_mask=None, lvl=0):
        """
        ...
        for i, adapter_stack_layer in enumerate(adapter_setup):
            ...
            if isinstance(adapter_stack_layer, Fuse):
                #if layer_id == 11:
                #    pdb.set_trace()
                hidden_states, attention_probs = self.adapter_fusion(
                    adapter_stack_layer, hidden_states, input_tensor, layer_norm, lang_vecs, layer_id, adapter_weights=adapter_weights, attention_mask=attention_mask, lvl=lvl + 1
                )
            ...
            # Case 5: We have a single adapter which is part of this module -> forward pass
                ...
                hidden_states, _, up = adapter_layer(hidden_states, residual_input=residual, layer_id=layer_id)
                ...
                if i == len(adapter_setup) - 1:
                    return hidden_states, up, input_tensor, attention_probs
    ...
    def adapter_fusion(self, adapter_setup: Fuse, hidden_states, input_tensor, layer_norm, lang_vecs, layer_id, adapter_weights=None, attention_mask=None, lvl=0):
        """
        ...
        for adapter_block in adapter_setup:
            ...
            # Case 2: We have a single adapter which is part of this module -> forward pass
                ...
                _, _, up = adapter_layer(hidden_states, residual_input=residual, layer_id=layer_id)
                up_list.append(up)
        ...
        if len(up_list) > 0:
            ...
            # if adapter_weights:
            #pdb.set_trace()
            hidden_states, attention_probs = self.adapter_fusion_layer[adapter_setup.name](
                query,
                up_list,
                up_list,
                residual,
    ...
    def adapter_layer_forward(self, hidden_states, input_tensor, layer_norm, lang_vecs, layer_id, adapter_names=None, adapter_weights=None, attention_mask=None):
        """
        ...
        if not skip_adapters and (len(set(self.adapters.keys()) & adapter_setup.flatten()) > 0):
            if adapter_weights is not None:
                assert adapter_setup[1] == 'ner' 
            FLAG = True
            input_hidden_states = hidden_states

            if isinstance(adapter_setup, Stack):
                hidden_states, _, input_tensor, adapter_weights = self.adapter_stack(
                    adapter_setup, hidden_states, input_tensor, layer_norm, lang_vecs, layer_id, adapter_names=adapter_names, adapter_weights=adapter_weights, attention_mask=attention_mask,
                )
                #attention_weights.append(attention_probs)
            elif isinstance(adapter_setup, Fuse):
                raise Exception("ERROR")
                hidden_states, attention_probs = self.adapter_fusion(adapter_setup, hidden_states, input_tensor, layer_norm, lang_vecs, layer_id)
