...
class RobertaSelfOutput(BertSelfOutputAdaptersMixin, nn.Module):
    ...
    def forward(self, hidden_states, input_tensor, lang_vecs, layer_id):
        hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.adapter_layer_forward(hidden_states, input_tensor, self.LayerNorm, lang_vecs, layer_id)
...
class RobertaAttention(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(self_outputs[0], hidden_states, lang_vecs, layer_id)
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        ...
        #pdb.set_trace()
        layer_output = apply_chunking_to_forward(self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, layer_id, attention_output, lang_vecs)
    ...
    def feed_forward_chunk(self, layer_id, attention_output, lang_vecs):
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output, lang_vecs, layer_id)
        return layer_output
...
class RobertaModel(BertModelAdaptersMixin, RobertaPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.l2v_mlp3 = LanguageMLP3(289,289,32,0.1)
    ...
    def forward(
        ...
        if type(embedding_output) == tuple:
            ...
            lang_vecs_ = torch.zeros(batch_size, 32).to('cuda')        
            adap_vecs_ = torch.zeros(len(adap_ids[0]), 32).to('cuda')
            for i in range(batch_size):
                #print(lang_ids)
                #pdb.set_trace()
                lang_vecs_[i] = self.l2v_mlp3(self.lang_features_[lang_ids[i].item()])
            
            for i in range(len(adap_ids[0])):
                adap_vecs_[i] = self.l2v_mlp3(self.lang_features_[adap_ids[0][i].item()])
            adap_vecs_ = adap_vecs_.unsqueeze(0).repeat(batch_size,1,1)
            concat_vecs_=torch.cat((lang_vecs_.unsqueeze(1),adap_vecs_),1)
