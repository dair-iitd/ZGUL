...
do
    for RF in 3;
    do
    OUTPUT_DIR="$OUT_DIR/$TASK/${MODEL}-LR${LR}-epoch${NUM_EPOCHS}-MaxLen${MAX_LENGTH}-TrainLang${TRAIN_LANGS}-Rf${RF}_${ADAPTER_LANG}_s${SEED}_ours_layerwise_dot/"
    mkdir -p $OUTPUT_DIR
    python third_party/run_train_pos.py \
    --do_train \
    --do_eval \
    --data_dir $DATA_DIR \
